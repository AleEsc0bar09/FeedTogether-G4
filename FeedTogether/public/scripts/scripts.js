...
function renderNavbar(usuario) {
  const rolActual = usuario ? usuario.rol : "guest";
  const items = document.querySelectorAll("#navbarLinks li[data-rol]");

  items.forEach((item) => {
    const rolesPermitidos = item.getAttribute("data-rol").split(",");
...
async function verificarSesionInicial() {
  try {
    ...
    renderNavbar(data.authenticated ? data.usuario : null);
  } catch (error) {
    renderNavbar(null);
  }
}
